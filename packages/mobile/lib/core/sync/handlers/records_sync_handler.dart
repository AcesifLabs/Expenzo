...
class RecordsSyncHandler extends SyncTableHandler<$RecordsTable, Record> {
  ...
  @override Map<String, dynamic> toSyncPayload(Record row) => {
    ...
    'date': row.date.toUtc().toIso8601String(), 'categoryId': row.categoryId,
  ...
  @override Insertable<Record> fromSyncPayload(String id, Map<String, dynamic> data) => RecordsCompanion.insert(
    id: id, amount: double.parse(data['amount'].toString()), description: data['description'] ?? '',
    date: DateTime.parse(data['date']).toLocal(), categoryId: data['categoryId'] != null ? Value(data['categoryId']) : const Value.absent(),
  ...
  @override Future<void> deleteById(AppDatabase db, String id) async => await (db.delete(db.records)..where((t) => t.id.equals(id))).go();
